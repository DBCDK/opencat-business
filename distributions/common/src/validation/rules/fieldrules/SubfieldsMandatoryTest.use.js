...
UnitTest.addFixture( "SubfieldsMandatory", function( ) {
    ...
    SafeAssert.equal( "1 testing with valid a param", [], SubfieldsMandatory.validateFields( record, field, paramsA ) );
    ...
    SafeAssert.equal( "2 testing with NON valid f param", errorFmissing, SubfieldsMandatory.validateFields( record, field, paramsF ) );

    var paramsABC = {
        'subfields' : ['a', 'b', 'c']
    };
    SafeAssert.equal( "3 testing with valid a,b,c param", [], SubfieldsMandatory.validateFields( record, field, paramsABC ) );

    errorFmissing = [ValidateErrors.fieldError( "TODO:url", errorMessage )];
    var paramsABF = {
        'subfields' : ['a', 'b', 'f']
    };
    SafeAssert.equal( "4 testing with NON valid  " + paramsABF.subfields + " param", errorFmissing, SubfieldsMandatory.validateFields( record, field, paramsABF ) );
    ...
    SafeAssert.equal( "5 testing with NON valid  " + paramsAXY.subfields + " param", errorXY, SubfieldsMandatory.validateFields( record, field, paramsAXY ) );
