...
START_TIME=`date '+%s'`
echo -n "Waiting for rawrepo-record-service $3 to be ready ";

echo "curl -m 5 http://${HOST}:${PORT}/api/howru 2>/dev/null"
for i in $(seq 1 ${TIMEOUT}) ; do
    RES=$(curl -m 5 http://${HOST}:${PORT}/api/howru 2>/dev/null)
