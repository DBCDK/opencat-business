...
public class MetaCompassHandler {
    ...
    private final static List<Character> metakompasSubFieldsToCopy = Arrays.asList('e', 'p');
     ...
     */
    public MarcRecord enrichMetaCompassRecord(MarcRecord minimalMetaCompassRecord) throws RecordServiceConnectorException, OpenCatException, ScripterException {
        ...
        if (subfieldsToCopy.size() > 0) {
            ...
            for (SubField subfieldToCopy : subfieldsToCopy) {
                ...
                if (!hasSubfield) {
                    ...
                    newField.addSubField(subfieldToCopy);
                    record.getFields(DataField.class).add(newField);
