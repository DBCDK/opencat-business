...
public class PreProcessingHandler {
    ...
    public void preProcess(MarcRecord record) throws Exception {
     ...
     * @param reader Reader for record
     */
    private void processFirstOrNewEdition(MarcRecord record, MarcRecordReader reader) throws Exception {
        ...
        if ("r".equals(subfield008u) && // Update edition
            ...
            // If someone updates the first edition record then 008 *u must be manually changed to the value u
     ...
     */
    private void processISBNFromPreviousEdition(MarcRecord record, MarcRecordReader reader) throws Exception {
     ...
     * @throws UnsupportedEncodingException If the previous record can't be decoded
     */
    private MarcRecordReader getHeadVolumeId(MarcRecordReader reader) throws Exception {
     ...
     * @throws UnsupportedEncodingException If the previous record can't be decoded
     */
    private void updateWithISBNFromPreviousEdition(MarcRecord record, MarcRecordReader reader) throws Exception {
     ...
     * @throws UnsupportedEncodingException If the previous record can't be decoded
     */
    private List<String> getISBNFromCommonRecord(String bibliographicRecordId) throws Exception {
    ...
    private void remove666UFields(MarcRecord record, List<Matcher> matchers) {
        ...
        List<DataField> fields = reader.getFieldAll("666");
        for (DataField field : fields) {
            List <SubField> subFields = field.getSubFields();
            for (SubField subField : subFields) {
                if ('u' == subField.getCode()) {
    ...
    private DataField getNewMarcField666(String value) {
        final SubField subfieldU = new SubField('u', value);

        DataField newField = new DataField("666", "00");
        newField.addSubField(subfieldU);
        return newField;
    }

    private void processSupplierRelations(MarcRecord record, MarcRecordReader reader) throws Exception {
        if (reader.hasSubfield("990", 'b') &&
            ...
            if (Arrays.asList("f", "c", "d", "o").contains(subfield008u) && !reader.hasSubfield("990", 'i')) {
                ...
                if (reader.hasValue("990", '&', "1")) {
                    ...
                    DataField add990 = new DataField("990", "00");
                    add990.addAllSubFields(newSubfields);
                    record.getFields().add(add990);
                }
            }
        }
    }
    ...
    private List<DataField> findField990(MarcRecordReader reader) {
        ...
        for (DataField field : reader.getFieldAll("990")) {
            if (field.hasSubField(hasSubFieldCode('r'))) {
                result.add(field);
            }
        }
